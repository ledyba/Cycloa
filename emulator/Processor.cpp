...
void Processor::updateFlagZN(const uint8_t& val)
{
...
// -- 以下、アドレッシングモード --
uint16_t Processor::addrImmediate()
{
...
void Processor:: BCS(uint16_t addr)
{
    if((this->P & FLAG_C) == FLAG_C){
        this->PC = addr;
    }
}
void Processor:: BEQ(uint16_t addr)
{
    if((this->P & FLAG_Z) == FLAG_Z){
        this->PC = addr;
    }
}
void Processor:: BNE(uint16_t addr)
{
    if((this->P & FLAG_Z) == 0){
        this->PC = addr;
    }
}
void Processor:: BVC(uint16_t addr)
{
    if((this->P & FLAG_V) == 0){
        this->PC = addr;
    }
}
void Processor:: BVS(uint16_t addr)
{
    if((this->P & FLAG_V) == FLAG_V){
        this->PC = addr;
    }
}
void Processor:: BPL(uint16_t addr)
{
    if((this->P & FLAG_N) == 0){
        this->PC = addr;
    }
...
void Processor:: JSR(uint16_t addr)
{
    this->PC--;
    push(static_cast<uint8_t>((this->PC >> 8) & 0xFF));
    push(static_cast<uint8_t>(this->PC & 0xFF));
...
void Processor:: JMP(uint16_t addr)
{
    this->PC = addr;
}
void Processor:: RTI()
